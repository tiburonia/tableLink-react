
const pool = require('../../shared/config/database');

// 카카오 REST API 키
const KAKAO_API_KEY = process.env.KAKAO_API_KEY;

if (!KAKAO_API_KEY) {
  console.error('❌ KAKAO_API_KEY 환경변수가 설정되지 않았습니다.');
  console.log('💡 Secrets 도구에서 KAKAO_API_KEY를 설정해주세요.');
  process.exit(1);
}

// API 호출 딜레이
const delay = (ms) => new Promise(resolve => setTimeout(resolve, ms));

// 전국 시군구 목록 (2024년 기준)
const CITY_OFFICES = [
  // 서울특별시 (25개 구)
  '서울특별시 종로구청', '서울특별시 중구청', '서울특별시 용산구청', '서울특별시 성동구청', '서울특별시 광진구청',
  '서울특별시 동대문구청', '서울특별시 중랑구청', '서울특별시 성북구청', '서울특별시 강북구청', '서울특별시 도봉구청',
  '서울특별시 노원구청', '서울특별시 은평구청', '서울특별시 서대문구청', '서울특별시 마포구청', '서울특별시 양천구청',
  '서울특별시 강서구청', '서울특별시 구로구청', '서울특별시 금천구청', '서울특별시 영등포구청', '서울특별시 동작구청',
  '서울특별시 관악구청', '서울특별시 서초구청', '서울특별시 강남구청', '서울특별시 송파구청', '서울특별시 강동구청',

  // 부산광역시 (16개 구군)
  '부산광역시 중구청', '부산광역시 서구청', '부산광역시 동구청', '부산광역시 영도구청', '부산광역시 부산진구청',
  '부산광역시 동래구청', '부산광역시 남구청', '부산광역시 북구청', '부산광역시 해운대구청', '부산광역시 사하구청',
  '부산광역시 금정구청', '부산광역시 강서구청', '부산광역시 연제구청', '부산광역시 수영구청', '부산광역시 사상구청',
  '부산광역시 기장군청',

  // 대구광역시 (8개 구군)
  '대구광역시 중구청', '대구광역시 동구청', '대구광역시 서구청', '대구광역시 남구청',
  '대구광역시 북구청', '대구광역시 수성구청', '대구광역시 달서구청', '대구광역시 달성군청',

  // 인천광역시 (10개 구군)
  '인천광역시 중구청', '인천광역시 동구청', '인천광역시 미추홀구청', '인천광역시 연수구청', '인천광역시 남동구청',
  '인천광역시 부평구청', '인천광역시 계양구청', '인천광역시 서구청', '인천광역시 강화군청', '인천광역시 옹진군청',

  // 광주광역시 (5개 구)
  '광주광역시 동구청', '광주광역시 서구청', '광주광역시 남구청', '광주광역시 북구청', '광주광역시 광산구청',

  // 대전광역시 (5개 구)
  '대전광역시 동구청', '대전광역시 중구청', '대전광역시 서구청', '대전광역시 유성구청', '대전광역시 대덕구청',

  // 울산광역시 (5개 구군)
  '울산광역시 중구청', '울산광역시 남구청', '울산광역시 동구청', '울산광역시 북구청', '울산광역시 울주군청',

  // 세종특별자치시 (1개)
  '세종특별자치시청',

  // 경기도 (31개 시군)
  '경기도 수원시청', '경기도 성남시청', '경기도 의정부시청', '경기도 안양시청', '경기도 부천시청',
  '경기도 광명시청', '경기도 평택시청', '경기도 동두천시청', '경기도 안산시청', '경기도 고양시청',
  '경기도 과천시청', '경기도 구리시청', '경기도 남양주시청', '경기도 오산시청', '경기도 시흥시청',
  '경기도 군포시청', '경기도 의왕시청', '경기도 하남시청', '경기도 용인시청', '경기도 파주시청',
  '경기도 이천시청', '경기도 안성시청', '경기도 김포시청', '경기도 화성시청', '경기도 광주시청',
  '경기도 양주시청', '경기도 포천시청', '경기도 여주시청', '경기도 연천군청', '경기도 가평군청', '경기도 양평군청',

  // 강원특별자치도 (18개 시군)
  '강원특별자치도 춘천시청', '강원특별자치도 원주시청', '강원특별자치도 강릉시청', '강원특별자치도 동해시청', '강원특별자치도 태백시청',
  '강원특별자치도 속초시청', '강원특별자치도 삼척시청', '강원특별자치도 홍천군청', '강원특별자치도 횡성군청', '강원특별자치도 영월군청',
  '강원특별자치도 평창군청', '강원특별자치도 정선군청', '강원특별자치도 철원군청', '강원특별자치도 화천군청', '강원특별자치도 양구군청',
  '강원특별자치도 인제군청', '강원특별자치도 고성군청', '강원특별자치도 양양군청',

  // 충청북도 (11개 시군)
  '충청북도 청주시청', '충청북도 충주시청', '충청북도 제천시청', '충청북도 보은군청', '충청북도 옥천군청',
  '충청북도 영동군청', '충청북도 증평군청', '충청북도 진천군청', '충청북도 괴산군청', '충청북도 음성군청', '충청북도 단양군청',

  // 충청남도 (15개 시군)
  '충청남도 천안시청', '충청남도 공주시청', '충청남도 보령시청', '충청남도 아산시청', '충청남도 서산시청',
  '충청남도 논산시청', '충청남도 계룡시청', '충청남도 당진시청', '충청남도 금산군청', '충청남도 부여군청',
  '충청남도 서천군청', '충청남도 청양군청', '충청남도 홍성군청', '충청남도 예산군청', '충청남도 태안군청',

  // 전북특별자치도 (14개 시군)
  '전북특별자치도 전주시청', '전북특별자치도 군산시청', '전북특별자치도 익산시청', '전북특별자치도 정읍시청', '전북특별자치도 남원시청',
  '전북특별자치도 김제시청', '전북특별자치도 완주군청', '전북특별자치도 진안군청', '전북특별자치도 무주군청', '전북특별자치도 장수군청',
  '전북특별자치도 임실군청', '전북특별자치도 순창군청', '전북특별자치도 고창군청', '전북특별자치도 부안군청',

  // 전라남도 (22개 시군)
  '전라남도 목포시청', '전라남도 여수시청', '전라남도 순천시청', '전라남도 나주시청', '전라남도 광양시청',
  '전라남도 담양군청', '전라남도 곡성군청', '전라남도 구례군청', '전라남도 고흥군청', '전라남도 보성군청',
  '전라남도 화순군청', '전라남도 장흥군청', '전라남도 강진군청', '전라남도 해남군청', '전라남도 영암군청',
  '전라남도 무안군청', '전라남도 함평군청', '전라남도 영광군청', '전라남도 장성군청', '전라남도 완도군청',
  '전라남도 진도군청', '전라남도 신안군청',

  // 경상북도 (23개 시군)
  '경상북도 포항시청', '경상북도 경주시청', '경상북도 김천시청', '경상북도 안동시청', '경상북도 구미시청',
  '경상북도 영주시청', '경상북도 영천시청', '경상북도 상주시청', '경상북도 문경시청', '경상북도 경산시청',
  '경상북도 군위군청', '경상북도 의성군청', '경상북도 청송군청', '경상북도 영양군청', '경상북도 영덕군청',
  '경상북도 청도군청', '경상북도 고령군청', '경상북도 성주군청', '경상북도 칠곡군청', '경상북도 예천군청',
  '경상북도 봉화군청', '경상북도 울진군청', '경상북도 울릉군청',

  // 경상남도 (18개 시군)
  '경상남도 창원시청', '경상남도 진주시청', '경상남도 통영시청', '경상남도 사천시청', '경상남도 김해시청',
  '경상남도 밀양시청', '경상남도 거제시청', '경상남도 양산시청', '경상남도 의령군청', '경상남도 함안군청',
  '경상남도 창녕군청', '경상남도 고성군청', '경상남도 남해군청', '경상남도 하동군청', '경상남도 산청군청',
  '경상남도 함양군청', '경상남도 거창군청', '경상남도 합천군청',

  // 제주특별자치도 (2개)
  '제주특별자치도 제주시청', '제주특별자치도 서귀포시청'
];

// 카카오 장소 검색 API 호출
async function searchOfficeLocation(officeName) {
  try {
    const response = await fetch(
      `https://dapi.kakao.com/v2/local/search/keyword.json?query=${encodeURIComponent(officeName)}`,
      {
        headers: {
          'Authorization': `KakaoAK ${KAKAO_API_KEY}`
        }
      }
    );

    if (!response.ok) {
      console.log(`❌ ${officeName} API 호출 실패: ${response.status}`);
      return null;
    }

    const data = await response.json();
    
    if (data.documents && data.documents.length > 0) {
      const place = data.documents[0]; // 첫 번째 결과 사용
      return {
        name: officeName,
        latitude: parseFloat(place.y),
        longitude: parseFloat(place.x)
      };
    }
    
    console.log(`⚠️ ${officeName} 검색 결과 없음`);
    return null;
    
  } catch (error) {
    console.error(`❌ ${officeName} 검색 실패:`, error.message);
    return null;
  }
}

// 지역명 파싱 함수
function parseRegionInfo(officeName) {
  const parts = officeName.split(' ');
  let regionType = '';
  let regionName = '';
  
  if (parts.length >= 2) {
    const sidoPart = parts[0];
    const sigunguPart = parts[1].replace('청', '');
    
    if (sidoPart.includes('특별시') || sidoPart.includes('광역시') || sidoPart.includes('특별자치시')) {
      regionType = 'sigungu';
      regionName = sigunguPart;
    } else if (sidoPart.includes('도')) {
      regionType = 'sigungu';
      regionName = sigunguPart;
    } else {
      regionType = 'sigungu';
      regionName = sidoPart + ' ' + sigunguPart;
    }
  }
  
  return { regionType, regionName };
}

async function importCityOffices() {
  try {
    console.log('🏛️ 전국 시청/군청/구청 좌표 가져오기 시작...');
    console.log(`📊 대상 관청 수: ${CITY_OFFICES.length}개`);
    
    // 기존 시군구 데이터만 삭제 (시도 데이터는 유지)
    await pool.query("DELETE FROM administrative_offices WHERE region_type = 'sigungu'");
    console.log('🗑️ 기존 시군구 데이터 삭제 완료');
    
    let successCount = 0;
    let failCount = 0;
    
    for (let i = 0; i < CITY_OFFICES.length; i++) {
      const officeName = CITY_OFFICES[i];
      
      console.log(`🔍 [${i + 1}/${CITY_OFFICES.length}] ${officeName} 검색 중...`);
      
      const result = await searchOfficeLocation(officeName);
      
      if (result) {
        try {
          // 지역 정보 파싱
          const { regionType, regionName } = parseRegionInfo(officeName);
          
          await pool.query(`
            INSERT INTO administrative_offices (region_type, region_name, office_name, latitude, longitude)
            VALUES ($1, $2, $3, $4, $5)
            ON CONFLICT (region_type, region_name) 
            DO UPDATE SET 
              office_name = EXCLUDED.office_name,
              latitude = EXCLUDED.latitude,
              longitude = EXCLUDED.longitude
          `, [regionType, regionName, result.name, result.latitude, result.longitude]);
          
          console.log(`✅ ${result.name}: (${result.latitude}, ${result.longitude})`);
          successCount++;
          
        } catch (error) {
          console.error(`❌ ${officeName} DB 저장 실패:`, error.message);
          failCount++;
        }
      } else {
        failCount++;
      }
      
      // API 호출 제한을 위한 딜레이 (200ms)
      await delay(200);
      
      // 진행상황 표시
      if ((i + 1) % 25 === 0) {
        console.log(`📊 진행상황: ${i + 1}/${CITY_OFFICES.length} (성공: ${successCount}, 실패: ${failCount})`);
      }
    }
    
    // 최종 결과 확인
    const totalResult = await pool.query("SELECT COUNT(*) as count FROM administrative_offices WHERE region_type = 'sigungu'");
    
    console.log(`\n🎉 전국 시청/군청/구청 좌표 가져오기 완료!`);
    console.log(`📊 최종 결과:`);
    console.log(`  - 성공: ${successCount}개`);
    console.log(`  - 실패: ${failCount}개`);
    console.log(`  - DB 저장된 총 시군구 관청 수: ${totalResult.rows[0].count}개`);
    
    // 전체 통계
    const allStats = await pool.query(`
      SELECT region_type, COUNT(*) as count 
      FROM administrative_offices 
      GROUP BY region_type 
      ORDER BY region_type
    `);
    
    console.log(`\n📈 전체 관청 통계:`);
    allStats.rows.forEach(stat => {
      console.log(`  - ${stat.region_type}: ${stat.count}개`);
    });
    
    // 검증: 목표 250개와 비교
    const actualCount = parseInt(totalResult.rows[0].count);
    const expectedCount = 250;
    
    console.log(`\n🔍 검증 결과:`);
    console.log(`  - 목표 시군구 관청 수: ${expectedCount}개`);
    console.log(`  - 실제 저장된 수: ${actualCount}개`);
    console.log(`  - 차이: ${Math.abs(expectedCount - actualCount)}개`);
    
    if (actualCount >= 240) { // 96% 이상이면 성공으로 간주
      console.log(`✅ 검증 성공! 충분한 수의 시군구 관청 좌표를 수집했습니다.`);
    } else {
      console.log(`⚠️ 검증 주의! 일부 시군구 관청 좌표를 가져오지 못했습니다.`);
    }
    
    // 샘플 데이터 확인
    const samples = await pool.query(`
      SELECT region_type, region_name, office_name, latitude, longitude 
      FROM administrative_offices 
      WHERE region_type = 'sigungu'
      ORDER BY RANDOM() 
      LIMIT 10
    `);
    
    console.log(`\n📍 샘플 시군구 데이터:`);
    samples.rows.forEach(office => {
      console.log(`  - ${office.office_name}: (${office.latitude}, ${office.longitude})`);
      console.log(`    지역: ${office.region_type} - ${office.region_name}`);
    });
    
  } catch (error) {
    console.error('❌ 시청/군청/구청 좌표 가져오기 실패:', error);
    console.error('❌ 에러 세부사항:', error.message);
  } finally {
    process.exit(0);
  }
}

// 스크립트 실행
importCityOffices();
