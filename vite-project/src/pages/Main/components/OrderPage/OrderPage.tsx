...
import { orderService, type MenuItem, type CartItem } from './services/orderService'
...
export const OrderPage = () => {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      const [storeData, menuData] = await Promise.all([
        orderService.getStoreInfo(storeId),
        orderService.getMenuList(storeId),
      ])
  ...
  // 카테고리 목록
  const categories = ['all', ...Array.from(new Set(menuList.map(m => m.category).filter(Boolean)))] as string[]
  ...
  // 수량 변경
  const handleUpdateQuantity = (menuId: string, change: number) => {
