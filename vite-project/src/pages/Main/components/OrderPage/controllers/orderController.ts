...
import { orderService, type CartItem, type MenuItem } from '../services/orderService'
...
export class OrderController {
   ...
   * 장바구니 수량 업데이트
   */
  updateQuantity(menuId: string, change: number, onUpdate: (cart: CartItem[]) => void) {
   ...
   * 장바구니에서 제거
   */
  removeFromCart(menuId: string, onUpdate: (cart: CartItem[]) => void) {
   ...
   * 주문 생성
   */
  async createOrder(storeId: string, tableNumber: number) {
