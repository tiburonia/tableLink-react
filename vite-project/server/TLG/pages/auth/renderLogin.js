...
window.quickLogin = async function(userId) {
  ...
  try {
    ...
    const response = await fetch('/api/auth/login', {
      ...
      body: JSON.stringify({ id: userId, password: 'cchcch11' })
    });
    ...
    if (loginInfoContainer) {
      loginInfoContainer.innerHTML = `
        ...
        <div class="signup-text-section">
          <p class="signup-text">
            TableLink가 처음이신가요? 
            <button type="button" class="signup-link-btn" id="signupCtaBtn">회원가입하러 가기</button>
          </p>
        </div>
      ...
      if (signupCtaBtn) {
        ...
        signupCtaBtn.addEventListener('click', handleSignUpClick);
      } else {
      ...
      if (goToSignUpBtn) {
        ...
        goToSignUpBtn.addEventListener('click', handleSignUpClick);
      } else {
