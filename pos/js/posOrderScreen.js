...
const POSOrderScreen = {
    ...
    renderTLLUserInfo() {
        ...
        // 카트에 아이템이 있는지 확인 (TLL 연동 버튼 표시 조건)
        ...
        return `
            <div class="tll-user-info">
                ...
                ${hasCartItems && hasTLLOrders ? `
                <div class="tll-integration-section">
                    <div class="integration-header">
                        <span class="integration-icon">🔗</span>
                        <span class="integration-title">TLL 연동 주문</span>
                    </div>
                    <div class="integration-description">
                        현재 카트에 ${this.cart.length}개 메뉴가 있습니다.<br>
                        기존 TLL 주문에 추가하시겠습니까?
                    </div>
                    <div class="integration-options">
                        <button class="tll-integration-btn merge-order" onclick="POSOrderScreen.showTLLIntegrationModal()">
                            <span class="btn-icon">➕</span>
                            <span class="btn-text">TLL 주문에 추가</span>
                        </button>
                        <div class="integration-help-text">
                            기존 TLL 주문과 함께 하나의 계산서로 결제됩니다.
                        </div>
                    </div>
                </div>
                ` : ''}
    ...
    showTLLIntegrationModal() {
        ...
        if (!this.cart || this.cart.length === 0) {
            alert('카트에 추가된 메뉴가 없습니다.');
            return;
        }

        // TLL 주문 총액 계산
        const tllTotal = this.tllOrders.reduce((sum, item) => sum + (item.total_price || 0), 0);
        const cartTotal = this.cart.reduce((sum, item) => sum + (item.price * item.quantity), 0);
        const combinedTotal = tllTotal + cartTotal;

        const modal = document.createElement('div');
        modal.className = 'tll-integration-modal-overlay';
        modal.innerHTML = `
            <div class="tll-integration-modal">
                <div class="modal-header">
                    <h2>🔗 TLL 연동 주문 선택</h2>
                    <button class="close-btn" onclick="this.closest('.tll-integration-modal-overlay').remove()">×</button>
                </div>
                ...
                <div class="modal-body">
                    <div class="integration-summary">
                        <div class="summary-section">
                            ...
                            <div class="order-summary">
                                <div class="customer-info">
                                    <span class="customer-name">${this.tllUserInfo?.name || '게스트'}</span>
                                    <span class="customer-phone">${this.tllUserInfo?.phone || this.tllUserInfo?.guest_phone || ''}</span>
                                </div>
                                <div class="order-details">
                                    <span class="item-count">${this.tllOrders.length}개 메뉴</span>
                                    <span class="order-amount">${tllTotal.toLocaleString()}원</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="summary-section">
                            ...
                            <div class="order-summary">
                                <div class="order-details">
                                    <span class="item-count">${this.cart.length}개 메뉴</span>
                                    <span class="order-amount">${cartTotal.toLocaleString()}원</span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="total-section">
                            <div class="combined-total">
                                ...
                                <span class="total-amount">${combinedTotal.toLocaleString()}원</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="integration-options">
                        <div class="option-card recommended">
                            <div class="option-header">
                                ...
                                <div class="option-title">기존 TLL 주문에 추가</div>
                            ...
                            <button class="option-btn primary" onclick="POSOrderScreen.processTLLIntegration(true)">
                                TLL 주문에 추가하기
                            </button>
                        </div>
                        ...
                        <div class="option-card">
                            <div class="option-header">
                                <div class="option-icon">🆕</div>
                                <div class="option-title">별도 POS 주문 생성</div>
                            </div>
                            <div class="option-description">
                                • 독립적인 POS 주문으로 처리<br>
                                • 별도 계산서 및 결제<br>
                            ...
                            <button class="option-btn secondary" onclick="POSOrderScreen.processTLLIntegration(false)">
                                별도 주문으로 처리
                            </button>
                        </div>
                    </div>
        ...
        // 모달 스타일 추가
        if (!document.querySelector('#tllIntegrationModalStyles')) {
            const style = document.createElement('style');
            style.id = 'tllIntegrationModalStyles';
            style.textContent = `
                .tll-integration-modal-overlay {
                    position: fixed;
                    top: 0;
                    left: 0;
                    right: 0;
                ...
                .tll-integration-modal .modal-header {
                    padding: 24px;
                ...
                .tll-integration-modal .modal-header h2 {
                    margin: 0;
                    font-size: 20px;
                    font-weight: 700;
                    color: #1e293b;
                ...
                .tll-integration-modal .close-btn {
                    background: none;
                    border: none;
                    font-size: 28px;
                    color: #64748b;
                ...
                .tll-integration-modal .close-btn:hover {
                    background: #f1f5f9;
                    color: #374151;
                }

                .tll-integration-modal .modal-body {
                    padding: 24px;
                }
     ...
     * TLL 연동 처리
     */
    async processTLLIntegration(mergeWithExisting) {
        try {
            console.log(`🔗 TLL 연동 처리 시작: ${mergeWithExisting ? '기존 주문에 추가' : '별도 주문 생성'}`);
            ...
            // 모달 닫기
            const modal = document.querySelector('.tll-integration-modal-overlay');
            if (modal) modal.remove();

            // 로딩 표시
            const loadingToast = this.showToast('TLL 연동 주문 처리 중...', 0);

            const cartTotal = this.cart.reduce((sum, item) => sum + item.price * item.quantity, 0);

            // TLL 연동 주문 API 호출
            const response = await fetch('/api/pos/tll-integration/confirm', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    storeId: parseInt(storeId),
            ...
            if (!response.ok) {
                const errorData = await response.json();
                throw new Error(errorData.error || 'TLL 연동 주문 처리 실패');
            }

            const result = await response.json();
            console.log('✅ TLL 연동 주문 완료:', result);
            ...
            // 성공 메시지
            const integrationMethod = mergeWithExisting ? 'TLL 주문에 추가' : '별도 POS 주문 생성';
            this.showToast(`${integrationMethod} 완료! (티켓 ID: ${result.ticketId})`);

            console.log('✅ TLL 연동 처리 완료');

        } catch (error) {
            console.error('❌ TLL 연동 처리 실패:', error);
            alert(`TLL 연동 처리 중 오류가 발생했습니다: ${error.message}`);
        }
    },

    /**
     * 토스트 메시지 표시 (타임아웃 옵션 추가)
     */
