...
router.post('/check-customer', async (req, res) => {
  try {
    ...
    console.log(`🔍 POS 고객 정보 확인: ${phone}`);
    ...
    const userResult = await pool.query(`
      SELECT u.id, u.name, u.phone, u.created_at,
             COALESCE(SUM(uss.points), 0) as points,
             COALESCE(MAX(uss.level), 'BRONZE') as level,
             COUNT(DISTINCT uss.store_id) as visited_stores
      FROM users u
      LEFT JOIN user_store_stats uss ON u.id = uss.user_id
    ...
    if (userResult.rows.length > 0) {
      ...
      res.json({
        ...
        customer: {
          ...
          visitedStores: parseInt(customer.visited_stores),
      ...
      const guestResult = await pool.query(`
        SELECT phone, visit_count, created_at
        FROM guests
      ...
      if (guestResult.rows.length > 0) {
        ...
        try {
          const visits = typeof guest.visit_count === 'string' 
            ? JSON.parse(guest.visit_count) 
            : guest.visit_count || {};
          visitCount = Object.values(visits).reduce((sum, count) => sum + count, 0);
        } catch (e) {
          visitCount = 0;
        }
    ...
    console.error('❌ POS 고객 정보 확인 실패:', error);
    res.status(500).json({
      success: false,
      error: '고객 정보 확인 실패'
    });
