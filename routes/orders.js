  ...
  try {
    ...
    if (global.kdsWebSocket) {
      global.kdsWebSocket.broadcast(storeId, 'new-order', {
        ...
        customerName: customerName || (userId ? null : '게스트'),
        itemCount: items.length,
...
router.put('/kds/items/:itemId/status', async (req, res) => {
  ...
  try {
    ...
    // KDS 실시간 업데이트
    if (global.kdsWebSocket) {
      global.kdsWebSocket.broadcast(store_id, 'item-status-update', {
...
router.post('/pay/:checkId', async (req, res) => {
  ...
  try {
    ...
    // 8. 실시간 업데이트
    if (global.kdsWebSocket) {
      global.kdsWebSocket.broadcast(check.store_id, 'check-completed', {
    ...
    if (global.posWebSocket) {
      global.posWebSocket.broadcast(check.store_id, 'payment-completed', {
        checkId: parseInt(checkId),
        paymentId: paymentId,
        tableNumber: check.table_number,
        finalAmount: finalAmount
      ...
      global.posWebSocket.broadcastTableUpdate(check.store_id, {
        tableNumber: check.table_number,
        isOccupied: false,
