...
export const NotificationCard: React.FC<NotificationCardProps> = ({ notification, onRead }) => {
  ...
  return (
    ...
    >
      <div className={styles.notificationIcon} style={{ backgroundColor: `${color}20`, color }}>
        {icon}
      </div>
      <div className={styles.notificationContent}>
        <div className={styles.notificationHeader}>
          <h4 className={styles.notificationTitle}>{notification.title}</h4>
          <span className={styles.notificationTime}>{relativeTime}</span>
        </div>
        <p className={styles.notificationMessage}>{notification.message}</p>
        {additionalInfo && (
          <div className={styles.notificationMeta}>{additionalInfo}</div>
        )}
      </div>
      {!isRead && <div className={styles.notificationBadge}></div>}
    </div>
  );
};
