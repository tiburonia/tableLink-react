...
/**
 * [POST] /tll-integration/confirm - TLL 연동 주문 처리 API
 * POS 주문을 기존 TLL 주문에 추가하거나 별도 주문으로 생성
 */
router.post('/tll-integration/confirm', async (req, res) => {
  const client = await pool.connect();

  try {
    const { storeId, tableNumber, items, totalAmount, mergeWithExisting } = req.body;

    console.log(`🔗 TLL 연동 주문 처리: 매장 ${storeId}, 테이블 ${tableNumber}, 연동방식: ${mergeWithExisting ? '기존주문추가' : '별도주문생성'}, 아이템: ${items.length}개`);
    ...
    if (mergeWithExisting) {
      // 옵션 A: 기존 TLL 주문에 POS 주문 추가
      console.log('📱➕ 기존 TLL 주문에 POS 주문 추가');

      // 해당 테이블의 활성 TLL 주문 찾기
      const existingTLLOrderResult = await client.query(`
      ...
      if (existingTLLOrderResult.rows.length === 0) {
        await client.query('ROLLBACK');
        return res.status(404).json({
          success: false,
          error: '연동할 활성 TLL 주문을 찾을 수 없습니다'
        });
      }

      targetOrderId = existingTLLOrderResult.rows[0].order_id;
      console.log(`📋 기존 TLL 주문 ID: ${targetOrderId}`);
      ...
      await client.query(`
        ...
        SET total_price = COALESCE(total_price, 0) + $1,
            updated_at = NOW()
        WHERE id = $2
      `, [totalAmount, targetOrderId]);

      // 새로운 order_ticket 생성 (source='POS')
      const ticketResult = await client.query(`
        INSERT INTO order_tickets (
          order_id,
          store_id,
          ...
          'PENDING', 'POSTPAID', 'POS', $3, NOW(), 'UNPAID')
        RETURNING id, batch_no
      `, [targetOrderId, storeId, tableNumber]);

      ticketId = ticketResult.rows[0].id;
      batchNo = ticketResult.rows[0].batch_no;

      console.log(`📋 새 POS 티켓 생성: ${ticketId} (배치: ${batchNo})`);

    } else {
      // 옵션 B: 별도의 새로운 POS 주문 생성 (spare_processing_order_id 활용)
      console.log('🆕 별도 POS 주문 생성');
      ...
      console.log(`📋 새 POS 주문 생성: ${targetOrderId}`);

      // store_tables의 spare_processing_order_id에 할당
      ...
      if (tableUpdateResult.rowCount === 0) {
        console.warn(`⚠️ 테이블 업데이트 실패 또는 테이블 찾을 수 없음: 매장 ${storeId}, 테이블 ${tableNumber}`);
      } else {
        console.log(`📋 spare_processing_order_id 업데이트: 테이블 ${tableNumber}, 주문 ${targetOrderId}`);
      }
    ...
    const integrationMethod = mergeWithExisting ? '기존 TLL 주문에 추가' : '별도 POS 주문 생성';
    console.log(`✅ TLL 연동 주문 완료: ${integrationMethod}, 주문 ID ${targetOrderId}, 티켓 ID ${ticketId}`);
    ...
    res.json({
      ...
      message: `${integrationMethod} 완료`
    });

  } catch (error) {
    await client.query('ROLLBACK');
    console.error('❌ TLL 연동 주문 처리 실패:', error);
    res.status(500).json({
      success: false,
      error: 'TLL 연동 주문 처리 중 오류가 발생했습니다: ' + error.message
    });
  } finally {
    client.release();
  }
