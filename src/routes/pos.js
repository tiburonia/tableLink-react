...
/**
 * [GET] /stores/:storeId/table/:tableNumber/tll-integration-check - TLL 연동 가능 여부 확인
 */
router.get('/stores/:storeId/table/:tableNumber/tll-integration-check', async (req, res) => {
  try {
    const { storeId, tableNumber } = req.params;
    ...
    console.log(`🔗 TLL 연동 확인: 매장 ${parsedStoreId}, 테이블 ${parsedTableNumber}`);

    // 해당 테이블의 기존 TLL 주문 확인 (OPEN 상태만)
    const tllOrderResult = await pool.query(`
      SELECT 
        o.id as order_id,
        o.source,
        o.session_status,
      ...
      WHERE o.store_id = $1 
        ...
        AND o.source = 'TLL'
        AND o.session_status = 'OPEN'
      GROUP BY o.id, o.source, o.session_status, o.total_price, o.created_at
      ORDER BY o.created_at DESC
      LIMIT 1
    `, [parsedStoreId, parsedTableNumber]);

    const hasTLLOrder = tllOrderResult.rows.length > 0;
    
    const response = {
      success: true,
      hasExistingTLLOrder: hasTLLOrder,
      integrationAvailable: hasTLLOrder,
    ...
    if (hasTLLOrder) {
      ...
      response.existingTLLOrder = {
        orderId: tllOrder.order_id,
        source: tllOrder.source,
        sessionStatus: tllOrder.session_status,
        totalPrice: tllOrder.total_price,
        ticketCount: parseInt(tllOrder.ticket_count),
        createdAt: tllOrder.created_at
      };
      console.log(`✅ TLL 연동 가능: 기존 TLL 주문 ${tllOrder.order_id} 발견`);
    } else {
    ...
    res.status(500).json({
      success: false,
      error: 'TLL 연동 확인 실패: ' + error.message
    });
  }
});

/**
 * [POST] /stores/:storeId/table/:tableNumber/tll-integration - TLL-POS 연동 처리
 */
router.post('/stores/:storeId/table/:tableNumber/tll-integration', async (req, res) => {
  const client = await pool.connect();
  ...
  try {
    ...
    console.log(`🔗 TLL-POS 연동 시작: 매장 ${parsedStoreId}, 테이블 ${parsedTableNumber}, ${items.length}개 아이템`);
    ...
    if (tllOrderResult.rows.length === 0) {
      ...
      return res.status(404).json({
        success: false,
        error: '연동할 TLL 주문을 찾을 수 없습니다'
      });
    }

    const tllOrder = tllOrderResult.rows[0];
    console.log(`🔍 기존 TLL 주문 발견: ${tllOrder.id}`);

    // 2. TLL 주문의 source를 'MIXED'로 변경 및 총액 업데이트
    await client.query(`
      UPDATE orders 
      SET source = 'MIXED',
          total_price = COALESCE(total_price, 0) + $1,
          updated_at = CURRENT_TIMESTAMP
      WHERE id = $2
    `, [totalAmount, tllOrder.id]);

    console.log(`✅ TLL 주문을 MIXED로 변경: ${tllOrder.id}, 추가 금액: ${totalAmount}`);

    // 3. store_tables의 spare_processing_order_id 업데이트
    const tableUpdateResult = await client.query(`
      UPDATE store_tables 
      SET spare_processing_order_id = $1,
          updated_at = CURRENT_TIMESTAMP
    ...
    `, [tllOrder.id, parsedStoreId, parsedTableNumber]);

    if (tableUpdateResult.rows.length > 0) {
      console.log(`📋 store_tables 업데이트 완료: spare_processing_order_id = ${tllOrder.id}`);
    } else {
      // table_number 필드로 재시도
      await client.query(`
        UPDATE store_tables 
        ...
        WHERE store_id = $2 AND table_number = $3
      `, [tllOrder.id, parsedStoreId, parsedTableNumber]);
      console.log(`📋 store_tables 업데이트 완료 (table_number 매칭): spare_processing_order_id = ${tllOrder.id}`);
    }

    // 4. 새로운 POS order_ticket 생성
    const ticketResult = await client.query(`
      INSERT INTO order_tickets (
        order_id,
        store_id,
        ...
        (SELECT COALESCE(MAX(batch_no), 0) + 1 FROM order_tickets WHERE order_id = $1),
        'PENDING', 'POSTPAID', 'POS', $3, CURRENT_TIMESTAMP, 'UNPAID')
      RETURNING id, batch_no
    `, [tllOrder.id, parsedStoreId, parsedTableNumber]);

    const { id: ticketId, batch_no: batchNo } = ticketResult.rows[0];
    console.log(`🎫 새 POS 티켓 생성: ${ticketId}, 배치: ${batchNo}`);

    // 5. order_items 생성 (POS 소스로)
    for (const item of items) {
      await client.query(`
        INSERT INTO order_items (
          order_id,
          ...
          menu_id,
          store_id
        ) VALUES ($1, $2, $3, $4, $5, $6, 'PENDING', $7, CURRENT_TIMESTAMP, $8, $9)
      `, [
        tllOrder.id,     // TLL 주문 ID 사용
        ticketId,        // 새로 생성된 POS 티켓 ID
        item.name,
        item.price,
        item.quantity,
        item.price * item.quantity,
    ...
    console.log(`✅ TLL-POS 연동 완료: 주문 ${tllOrder.id}, 티켓 ${ticketId}, 배치 ${batchNo}`);
    ...
    res.json({
      ...
      batchNo: batchNo,
      source: 'MIXED',
      message: 'TLL-POS 연동이 성공적으로 처리되었습니다'
    });

  } catch (error) {
    await client.query('ROLLBACK');
    console.error('❌ TLL-POS 연동 실패:', error);
    res.status(500).json({
      success: false,
      error: 'TLL-POS 연동 처리 실패: ' + error.message
    });
  } finally {
    client.release();
  }
