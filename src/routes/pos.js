...
/**
 * [GET] /stores/:storeId/table/:tableNumber/tll-unpaid-orders - 테이블별 TLL 미지불 주문 조회 (POS 화면용)
 */
router.get('/stores/:storeId/table/:tableNumber/tll-unpaid-orders', async (req, res) => {
  try {
    const { storeId, tableNumber } = req.params;

    console.log(`📱 TLL 미지불 주문 조회: 매장 ${storeId}, 테이블 ${tableNumber}`);

    // TLL 미지불 주문만 조회 (source='TLL', paid_status='UNPAID')
    ...
    // 사용자 정보 조회 (첫 번째 주문의 사용자 정보 사용)
    let userInfo = null;
    if (orderItemsResult.rows.length > 0) {
      const firstOrderId = orderItemsResult.rows[0].order_id;
      
      const orderInfoResult = await pool.query(`
        SELECT o.user_id, o.guest_phone, u.name, u.phone, u.point, u.created_at
        FROM orders o
        LEFT JOIN users u ON o.user_id = u.id
        WHERE o.id = $1
      `, [firstOrderId]);
      ...
      if (orderInfoResult.rows.length > 0) {
        ...
        } else if (orderInfo.guest_phone) {
          // 게스트 주문인 경우
          userInfo = {
            id: null,
            name: '게스트',
            phone: orderInfo.guest_phone,
            guest_phone: orderInfo.guest_phone,
            point: 0,
            created_at: null
          };
        }
    ...
    console.log(`✅ TLL 미지불 주문 ${orderItemsResult.rows.length}개 조회 완료`);

    res.json({
      success: true,
      orderItems: orderItemsResult.rows,
      userInfo: userInfo,
      count: orderItemsResult.rows.length
    });

  } catch (error) {
    console.error('❌ TLL 미지불 주문 조회 실패:', error);
    res.status(500).json({
      success: false,
      error: 'TLL 미지불 주문 조회 실패: ' + error.message
    });
  }
});
