...
router.post('/checks/from-qr', async (req, res) => {
  ...
  try {
    ...
    const storeId = 1; // 현재는 매장 ID를 1로 고정
    ...
    if (existingCheckResult.rows.length > 0) {
      ...
      console.log(`✅ TLL 기존 체크 사용: ${checkId} (테이블 ${tableNumber})`);
    } else {
      // 새 체크 생성
      ...
      console.log(`✅ TLL 새 체크 생성: ${checkId} (테이블 ${tableNumber})`);
    }
...
/**
 * [POST] /payments/confirm - TLL 결제 확인 처리
 */
  ...
  try {
    ...
    if (paymentUpdateResult.rows.length === 0) {
      ...
      await client.query(`
        INSERT INTO payments (
          check_id, method, amount, status,
          completed_at, pg_transaction_id, payment_data
        )
    ...
    // TLL 회원 포인트 적립 (users 테이블에 point 컬럼이 있다면)
    if (check.user_id) {
      ...
      if (points > 0) {
        try {
          ...
          console.warn('⚠️ 포인트 적립 실패:', pointError.message);
        }
      }
    }
...
router.get('/checks/:checkId', async (req, res) => {
  try {
    ...
    const result = await pool.query(`
      ...
      LEFT JOIN store_info si ON c.store_id = si.store_id
      LEFT JOIN check_items ci ON c.id = ci.check_id AND ci.status != 'canceled'
      WHERE c.id = $1
      GROUP BY c.id, si.name, si.category
    `, [checkId]);
    ...
    res.json({
      ...
      check: {
        ...
        storeName: checkData.store_name || '매장',
