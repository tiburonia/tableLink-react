...
/**
 * [GET] /checks/:id/status - TLL 체크 상태 조회
 */
router.get('/checks/:id/status', async (req, res, next) => {
  const client = await pool.connect();
  ...
  try {
    ...
    // 체크 존재 및 매장 스코프 확인
    const checkResult = await client.query(`
      SELECT id, store_id, status, table_number
      FROM checks 
      WHERE id = $1
    `, [checkId]);
    ...
    const linesResult = await client.query(`
      SELECT ol.id, ol.menu_name, ol.status, ol.created_at
      FROM order_lines ol
      JOIN orders o ON ol.order_id = o.id
